-- ============================================================
-- MIGRATION 027: Production article comments schema + RLS
--
-- Goal:
-- - Stabilize article_comments for threaded, realtime comments
-- - Enforce schema required by mobile app insert payload
-- - Ensure secure read/write with explicit RLS policies
-- ============================================================

CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Ensure base table exists with core required columns.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'article_comments_pkey'
      AND conrelid = 'article_comments'::regclass
  ) THEN
    ALTER TABLE article_comments ADD CONSTRAINT article_comments_pkey PRIMARY KEY (id);
  END IF;
END $$;

-- Normalize user_id column to uuid safely (drop non-uuid guest rows).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
-- Ensure created_at type is timestamptz.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
-- Rebuild critical foreign keys with cascade delete.
DO $$
DECLARE
  fk record;
BEGIN
...
CREATE POLICY article_comments_select_public
  ON article_comments
  FOR SELECT
  USING (true);

CREATE POLICY article_comments_insert_authenticated
  ON article_comments
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY article_comments_update_owner
  ON article_comments
  FOR UPDATE
  TO authenticated
...
CREATE POLICY article_comments_delete_owner
  ON article_comments
  FOR DELETE
  TO authenticated
  USING (auth.uid() = user_id);
